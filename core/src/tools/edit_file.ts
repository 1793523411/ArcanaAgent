  ...
  },
  {
    ...
    schema: z.object({
      ...
      edits: z.array(z.object({
        ...
        start_line: z.number().optional().nullable().describe("Start line number (1-based). Narrows search scope to this range"),
        end_line: z.number().optional().nullable().describe("End line number (1-based, inclusive). Used with start_line"),
      })).min(1).describe("Array of search-and-replace operations, applied sequentially"),
      dry_run: z.boolean().optional().nullable().describe("If true, preview changes without writing (default false)"),
    }),
  }
);
