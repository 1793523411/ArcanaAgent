...
export const web_search = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      type: z.enum(["web", "news"]).optional().nullable().describe("Search type (default: web)"),
      max_results: z.number().optional().nullable().describe("Maximum number of results (default 5, max 20)"),
      time_range: z.enum(["d", "w", "m", "y"]).optional().nullable().describe("Time range filter: d=day, w=week, m=month, y=year"),
    }),
  }
);
