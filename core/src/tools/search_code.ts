...
export const search_code = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      path: z.string().optional().nullable().describe("Directory or file to search in (defaults to workspace root)"),
      file_glob: z.string().optional().nullable().describe("File glob filter, e.g. '*.ts', '*.{py,js}'"),
      case_sensitive: z.boolean().optional().nullable().describe("Case sensitive search (default true)"),
      context_lines: z.number().optional().nullable().describe("Context lines before and after each match (default 2, max 10)"),
      max_results: z.number().optional().nullable().describe("Maximum number of matching lines (default 50, max 200)"),
      mode: z.enum(["text", "symbol"]).optional().nullable().describe("Search mode: 'text' for regex content search (default), 'symbol' for function/class/interface definitions only"),
    }),
  }
);
