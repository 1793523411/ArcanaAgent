...
export const git_operations = tool(
  ...
  },
  {
    ...
    schema: z.object({
      ...
      args: z.object({
        ref: z.string().optional().nullable().describe("Ref to diff against, e.g. 'HEAD~1', 'main'"),
        files: z.array(z.string()).optional().nullable().describe("Specific files to operate on"),
        max_count: z.number().optional().nullable().describe("Number of log entries (default 10, max 50)"),
        message: z.string().optional().nullable().describe("Commit message (required for commit)"),
        branch_name: z.string().optional().nullable().describe("Branch name (for create_branch)"),
      }).optional().nullable(),
      working_directory: z.string().optional().nullable().describe("Git repo path (defaults to workspace)"),
    }),
  }
);
