...
| `modelAdapter` | `ModelAdapter` | 否 | — | 自定义模型适配器，传入后将忽略 `model` 配置（高级用法） |
...
| `agentTimeoutMs` | `number` | 否 | `0`（不限制） | Agent 执行超时时间（毫秒），超时后自动中止 |
...
手动初始化 Agent，主要用于提前连接 MCP 服务器等异步资源。

- 多次调用安全：内部使用 Promise 去重，并发调用只会执行一次实际初始化
- `stream()` 和 `run()` 会在首次调用时自动触发 `init()`，大多数场景下不需要手动调用
- 如需确保 MCP 连接就绪后再执行任务，可显式调用 `await agent.init()`
